...
%LOAD INDEXES BEFORE RUNNING

load("mode_times.mat") 
trange = ['2023-11-05';'2023-11-05'];
...
for i = start_datenum:end_datenum
    ...
    if fileID_CIRBE_1 > 0
        ...
        Datafilename = strjoin(["C:\Users\2001d\Desktop\LASP Data\Data release\netcdf file\daily_netcdf_v2\CIRBE_REPTile-2_L1_",filedate,"v2_0.nc"],"");
        ...
        for p = 2:num_t
            ...
            if((IntePrd(p)==5000) && (0.05>abs(1-timediff))) %occasionally when integration period changes, 1 second integration period datapoints are mislabeled as 5 second datapoints
                IntePrd(p) = 1000;
            elseif(inttime>timediff) %datapoints where the elapsed time from the previous datapoint is less than the integration period should be impossible and are therefore considered invalid
                valflag(p)=1;
            end
        end
...
for i = start_datenum:end_datenum
    ...
    if fileID_CIRBE_1 > 0
        ...
        timemin = double(convertTo(currentday,'epochtime','Epoch','2000-01-01','TicksPerSecond',1000))/1000;
        timemax = timemin+86399.999; %milisecond before midnight
