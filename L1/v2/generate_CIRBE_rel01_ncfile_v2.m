load("mode_times.mat") 
%trange = ['2023-11-05';'2023-12-31'];
%trange = ['2024-03-01';'2024-03-31'];
trange = ['2024-07-02';'2024-08-25'];
...
for i = start_datenum:end_datenum
    ...
    if fileID_CIRBE_1 > 0
        ...
        Datafilename = strjoin(["C:\Users\2001d\OneDrive\Desktop\LASP Data\Data release\netcdf file\daily_netcdf_v2\CIRBE_REPTile-2_L1_",filedate,"v2_0.nc"],"");

        timeraw = double(ncread(file_path_2,"time_ymdhms"))';
        time = datetime(timeraw);
        t = double(convertTo(time,'epochtime','Epoch','2000-01-01','TicksPerSecond',1000))/1000; %seconds since 2000-01-01, specify 1000 for milisecond precision and to prevent rounding
        num_t = length(time);
        ...
        for p = 2:num_t
            timediff = t(p)-t(p-1); %integration period of a certain datapoint (when there are no data gaps) should be the time between the current datapoint and the previous one
            if((IntePrd(p)==5000) && (0.05>abs(1-timediff))) %occasionally when integration period changes, 1 second integration period datapoints are mislabeled as 5 second datapoints
                IntePrd(p) = 1000;
                integrationswitches(q) = p;
                q=q+1;
            end
            inttime = (IntePrd(p)/1000)*0.9; %threshold for invalidity is 90 percent, allowing for minor variations in cadence
            if(inttime>timediff) %datapoints where the elapsed time from the previous datapoint is less than the integration period should be impossible and are therefore considered invalid
                valflag(p)=1;
            end
        end

        save(file_datestring,"integrationswitches")
        ...
        HK_time = datenum('1999/12/31 23:59:23')+(HK_time_raw/1000/3600/24); %this specific time is used because the "Pointing_timestamp" timestamps are in TAI, exactly 37 seconds behind UTC
...
for i = start_datenum:end_datenum
    ...
    if fileID_CIRBE_1 > 0
        ...
        filedate = datestr(i,"yyyymmdd");
        Datafilename = strjoin(["C:\Users\2001d\OneDrive\Desktop\LASP Data\Data release\netcdf file\daily_netcdf_v2\CIRBE_REPTile-2_L1_",filedate,"v2_0.nc"],"");
        currentday = datetime(i,'ConvertFrom','datenum');
        timemin = double(convertTo(currentday,'epochtime','Epoch','2000-01-01'));
        ...
        ncwriteatt(Datafilename,"/",'Data_version','2.0');
        ...
        ncwriteatt(Datafilename,"L","CATDESC","McIlwain's L-parameter");
        ...
        ncwriteatt(Datafilename,"MLT","LABLAXIS","Magnetic Local Time (decimal hours)");
        ncwriteatt(Datafilename,"MLAT","LABLAXIS","Magnetic Latitude (degrees)");
        ...
        % VALIDMIN
        ncwriteatt(Datafilename,"Epoch","VALIDMIN",timemin);
        ...
        ncwriteatt(Datafilename,"IntePrd","VALIDMIN",100);
        ncwriteatt(Datafilename,"OffAng_timestamp","VALIDMIN",timemin);
        ...
        % VALIDMAX 
        ncwriteatt(Datafilename,"Epoch","VALIDMAX",timemax);
        ncwriteatt(Datafilename,"Alt","VALIDMAX",600);
        ncwriteatt(Datafilename,"Lat","VALIDMAX",90);
        ncwriteatt(Datafilename,"Lon","VALIDMAX",180);
        ncwriteatt(Datafilename,"L","VALIDMAX",1000); % above ~12 these numbers have no real physical meaning
        ...
        ncwriteatt(Datafilename,"IntePrd","VALIDMAX",5000);
        ncwriteatt(Datafilename,"OffAng_timestamp","VALIDMAX",timemax);
        ...
        ncwriteatt(Datafilename,"IntePrd","SCALEMIN",100)
        ...
        ncwriteatt(Datafilename,"IntePrd","SCALEMAX",5000)
